...
function renderHome(snapshot) {
  ...
  return `<section class="ielts-page ielts-home view-enter">
    ...
    <div class="ielts-mode-grid">
      ...
      <button class="ielts-mode-card ielts-full-mock panel" type="button" data-action="ielts-select-mode" data-mode="full_mock"><span>FULL MOCK · 11–14 MIN</span><h2>完整模拟考试</h2><p>Part 1 → Part 2 → Part 3，无暂停、跳题或重试。Demo 使用加速题量和准备计时。</p><b>进入完整模考 →</b></button>
    </div>
  </section>`;
}
...
function renderPreflight(snapshot) {
  ...
  return `<section class="ielts-page ielts-preflight view-enter">
    ...
    <div class="ielts-preflight-grid">
      <section class="ielts-preflight-main panel"><p class="eyebrow">BEFORE YOU START</p><h1>${selectionName(snapshot.selection)}</h1><p>请先确认本次练习规则。Demo 的 Part 2 准备倒计缩短为 10 秒，领域规则仍保留正式 60 秒配置。</p>
...
const STATUS_META = {
  ...
  part2_rounding_off: ["Part 2", "收尾问题"],
...
function cueCardMarkup(card) {
  return `<article class="ielts-cue-card" data-cue-card-id="${escapeHtml(card.cardId)}">
    ...
    <div class="ielts-cue-body"><strong>You should say:</strong><ul>${card.youShouldSay.map((item) => `<li>${escapeHtml(item)}</li>`).join("")}</ul><p>${escapeHtml(card.explain)}</p></div>
  </article>`;
}
...
function questionStage(snapshot, item) {
  ...
  return `<section class="ielts-listening-stage panel">
    ...
    <h2>${practice ? "回答当前问题" : "请仔细听考官提问"}</h2>
    ${practice ? `<p class="ielts-practice-question" data-current-question>${escapeHtml(item?.renderedText || "")}</p>` : `<p>完整模考不显示独立题卡。如需文字辅助，可开启字幕。</p>`}
  </section>`;
}
...
function answerControls(snapshot, voice = DEFAULT_VOICE_SNAPSHOT) {
  ...
  return `<section class="ielts-voice-panel panel is-${voice.status}" aria-label="语音回答">
    <header><div><p class="eyebrow">YOUR ANSWER</p><h3>用英语回答当前问题</h3></div><time>${formatTime(voice.elapsedSeconds)}</time></header>
    ...
    <label class="ielts-transcript" for="ielts-voice-transcript"><span>实时转写 · 可手动修正</span><textarea id="ielts-voice-transcript" data-action="ielts-voice-transcript" placeholder="开始说话后，转写会显示在这里…">${escapeHtml(voice.finalTranscript || "")}</textarea>${voice.interimTranscript ? `<small>${escapeHtml(voice.interimTranscript)}</small>` : ""}</label>
    <footer>${practiceActions}<button class="primary-btn" type="button" data-action="ielts-voice-finish"${voice.status === "finalizing" ? " disabled" : ""}>结束本轮回答</button></footer>
  </section>`;
}
...
function renderSession(snapshot, voiceSnapshot) {
  ...
  return `<section class="ielts-page ielts-session view-enter">
    <header class="ielts-session-head">
      <div><span class="ielts-demo-badge">DEMO 加速模式</span><p><strong>${meta[0]}</strong><small>${meta[1]}</small></p></div>
...
function renderReport(snapshot) {
  ...
  return `<section class="ielts-page ielts-report view-enter">
    <header class="ielts-report-hero"><p class="eyebrow">AI PRACTICE FEEDBACK</p><h1>AI 练习反馈</h1><p>${escapeHtml(report?.disclaimer || "")}</p><div><span>${report?.summary?.answerCount || 0} 段回答</span><span>字幕${report?.summary?.captionsUsed ? "已使用" : "未使用"}</span><span>录音${report?.summary?.recordingEnabled ? "已选择" : "未开启"}</span></div></header>
    <div class="ielts-dimension-grid">${dimensions.map((dimension) => `<article class="ielts-dimension-card panel ${dimension.status === "not_assessed" ? "is-muted" : ""}"><div><span>${dimension.status === "not_assessed" ? "未评估" : "已生成证据"}</span><h2>${escapeHtml(dimension.label)}</h2></div><dl><dt>表现证据</dt><dd>${escapeHtml(dimension.evidence)}</dd><dt>本次优点</dt><dd>${escapeHtml(dimension.strength)}</dd><dt>下一步</dt><dd>${escapeHtml(dimension.nextStep)}</dd></dl></article>`).join("")}</div>
