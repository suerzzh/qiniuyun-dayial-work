...
const stateControlledScenario = buildRealtimeSessionConfig({
  systemPrompt: "Stay in role.",
  automaticTurnResponses: false,
});
assert.equal(stateControlledScenario.turn_detection.create_response, false);
assert.equal(slowerKaterina.turn_detection.create_response, true);
